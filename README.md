...
- `scripts/evaluate-scoresheets.ts` — checks `parseImage` against real scoresheet photos with known-correct games, outside `npm test` (see [Known accuracy limitations](#handwritten-scoresheet-recognition)).
...
**Known accuracy limitations (open, not yet addressed)**: evaluated against all four real 1963 World Championship scoresheet photos in `data/` (see [scripts/evaluate-scoresheets.ts](scripts/evaluate-scoresheets.ts) — checks recognized output against the actual games in `data/worldchamp1963.pgn`, keyed by the `[Round "N"]` tag matching each `1963-N.jpg` filename; not part of `npm test` since it needs a real Gemini API key/network access and reads photos that live outside this repo). Three findings, run 3+ times per image since a single run isn't meaningful (see next point):

- **Gemini's OCR reading isn't deterministic between calls on the same image.** One round (15) went from stopping at move 21 to fully correct to stopping at move 27 across three otherwise-identical runs. A single evaluation run is not reliable evidence of either success or failure.
- **A misread that happens to *also* be a legal move is invisible to the current stop-and-warn mechanism.** Round 5 consistently (6/6 runs across two prompt variants) recognizes `Re6` where the real game has `Rc6` around move 89 — a `c`→`e` letter misread, but since `Re6` is a fully legal move in that exact position, there's no tie and nothing illegal to catch; recognition continues confidently down the wrong branch with no warning at all. This is a different, currently-unsolved gap from the "stops and reports" cases above.
- **A targeted prompt fix for this was tried and made things worse.** Added explicit guidance to double-check `c`/`e` confusion and count `O-O` vs `O-O-O` dashes carefully. Measured over 3 runs each: Round 5's error was completely unaffected (same `Re6`/`Rc6` misread every time, prompt or no), Round 15 stayed inconsistently wrong, and Round 21 — 100% reliable in every prior test all session — dropped to 1/3 correct, misreading the very first move (`c4` → `e4`) twice. Priming the model to second-guess a specific letter pair appears to reduce its confidence on genuinely unambiguous instances of that same letter, not just fix the intended cases. Reverted; not worth attempting again without a different angle.
